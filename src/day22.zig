...
const Map = struct {
    ...
    fn init() Self {
        return .{ .cubes = std.ArrayList(Cube).init(std.testing.allocator) };
    }
...
fn part1() !i64 {
    ...
    while (it.next()) |item| {
        ...
        const x_min = try std.fmt.parseInt(i64, xparts.next().?, 10);
        const x_max = try std.fmt.parseInt(i64, xparts.next().?, 10);

        var yparts = std.mem.tokenize(u8, parts.next().?[2..], ".");
        const y_min = try std.fmt.parseInt(i64, yparts.next().?, 10);
        const y_max = try std.fmt.parseInt(i64, yparts.next().?, 10);

        var zparts = std.mem.tokenize(u8, parts.next().?[2..], ".");
        const z_min = try std.fmt.parseInt(i64, zparts.next().?, 10);
        const z_max = try std.fmt.parseInt(i64, zparts.next().?, 10);
