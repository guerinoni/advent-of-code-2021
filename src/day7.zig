...
fn part1(positions: []i64) !i64 {
    const median = positions[positions.len / 2];
    var fuel: i64 = 0;
    for (positions) |pos| {
        fuel += std.math.absInt(pos - median) catch unreachable;
    }
...
fn part2(positions: []i64) !i64 {
    ...
    var avg = @divFloor(sum, @intCast(i64, positions.len));
    var fuel: i64 = 0;
    for (positions) |pos| {
        var diff = std.math.absInt(pos - avg) catch unreachable;
        fuel += @divTrunc(diff * (diff + 1), 2);
    }
    return fuel;
}
